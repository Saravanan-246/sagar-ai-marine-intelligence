...
export default function MapLibreMapAdapter({
  ...
  useEffect(() => {
    ...
    const segmentsFeatures = segments.map((seg) => {
      ...
      const coords = [
        [seg.startCoord[1], seg.startCoord[0]],
        [seg.endCoord[1], seg.endCoord[0]],
      ];
    ...
    if (segments && segments.length > 0) {
      try {
        ...
        segments.forEach((seg) => {
          if (seg.startCoord) {
            minLat = Math.min(minLat, seg.startCoord[0]);
            maxLat = Math.max(maxLat, seg.startCoord[0]);
            minLon = Math.min(minLon, seg.startCoord[1]);
            maxLon = Math.max(maxLon, seg.startCoord[1]);
          }
          if (seg.endCoord) {
            minLat = Math.min(minLat, seg.endCoord[0]);
            maxLat = Math.max(maxLat, seg.endCoord[0]);
            minLon = Math.min(minLon, seg.endCoord[1]);
            maxLon = Math.max(maxLon, seg.endCoord[1]);
          }
        });
